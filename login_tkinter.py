...
def login():
	...
	if user_name.get()=="" or password.get()=="":
		...
		try:
			conn.connect(host="localhost",user="root",password="",database="docterapp")
			cur = con.cursor()
...
def deshboard():
	...
	def book():
		if docter_var.get() =="" or day.get() =="" or month.get() == "" or year.get() == "":
			...
			con = sqlite3.connect(host="localhost",user="root",password="",database="docterapp")
			cur = con.cursor()
	...
	def action():
		if first_name.get()=="" or last_name.get()=="" or age.get()=="" or city.get()=="" or add.get()=="" or user_name.get()=="" or password.get()=="" or very_pass.get()=="":
			...
			try:
				...
				messagebox.showerror("Error" , f"Error con hai Dui to : {str(es)}", parent = winsignup)
